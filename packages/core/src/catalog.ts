...
export const TOOLS: ToolDef[] = [
  ...
  // ─────────────────────────────────────────────────────────────────────────
  {
    ...
    name: "Claude Code",
    shortName: "CC",
    products: ["CLI", "Claude Desktop", "VS Code / JetBrains extension"],
    blurb:
      "Anthropic Claude Code CLI and Claude Desktop. CLI state lives in ~/.claude (CLAUDE_CONFIG_DIR), and Claude Desktop stores app cache in AppData / Application Support.",
    notes: [
      "Sessions are JSONL under projects/<encoded-cwd>/<session-id>.jsonl.",
      "Default retention is cleanupPeriodDays = 30.",
      "file-history/ stores edit snapshots for rollback and can be cleaned to free space.",
      "claude_desktop_config.json (MCP) and MEMORY.md are strictly protected.",
    ],
    targets: [
      {
        id: "cc-sessions",
        label: "Project & CLI sessions",
        kind: "conversations",
        risk: "high",
        description: "JSONL transcripts, subagents, and spilled tool-results under ~/.claude/projects and ~/.claude/sessions.",
        paths: {
          win: ["%USERPROFILE%\\.claude\\projects", "%USERPROFILE%\\.claude\\sessions"],
          mac: ["~/.claude/projects", "~/.claude/sessions"],
          linux: ["~/.claude/projects", "~/.claude/sessions"],
        },
      },
      {
        id: "cc-history-extras",
        label: "History, pastes & file history",
        kind: "conversations",
        risk: "high",
        description: "history.jsonl (up-arrow recall), paste-cache, file-history (rewind snapshots), uploads, backups.",
        paths: {
          win: [
      ...
      {
        id: "cc-cache",
        label: "CLI cache & downloads",
        kind: "cache",
        risk: "low",
        description: "stats-cache.json, usage-data/, cache/ and downloads/ under ~/.claude. Does not affect transcripts.",
        paths: {
          win: [
